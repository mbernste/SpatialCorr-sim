...
def simulate_within_region_varying_correlation_pairwise(
        adata,
        gene_1,
        gene_2,
...
def simulate_region_specific_pairwise((
        adata,
        clust_to_corr,
        gene_1,
        gene_2,
    ...
    # Generate counts
    covs, sample = simulate_expression_guassian_no_spatial(
        means_g1,
        means_g2,
        vars_g1,
        vars_g2,
...
def simulate_gene_set_from_dataset_precomputed_cov( # HERE1
        adata,
        all_genes,
        spotwise_covs,
        row_key='array_row',
    ...
    corrs, covs, sample = simulate_precomputed_cov_mult_genes(
        spotwise_means,
        spotwise_covs,
        coords,
        poisson=poisson,
...
def simulate_expression_guassian_cov_mult_genes(
    ...
    for si in range(len(coords)):
        z = Z[si]
        cov = np.outer(z, z)
        mean = spot_latent_means.T[si]
       
        # Base covariance matrix
        U = np.zeros((G,G)) 
        np.fill_diagonal(U, spot_latent_vars.T[si])
        cov += U
        covs.append(cov)
...
def simulate_precomputed_cov_mult_genes(
        spotwise_means,
        spotwise_covs,
        coords,
        poisson=False,
    ...
    samples = []
    covs = spotwise_covs # For consistency with other methods
    for si in range(len(coords)):
        cov = covs[si]
        mean = spotwise_means[si]
...
def simulate_expression_guassian_no_spatial(
    ...
    x_covs = x_corrs * np.sqrt(x_vars_g1 * x_vars_g2)
    ...
    for s_i in range(len(coords)):
        ...
        sample.append(x_s)
    sample = np.array(sample).T
    #return x_corrs, sample
    return x_covs, sample
